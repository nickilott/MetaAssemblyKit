...
class Spades(Idba):
    ...
    def build(self, infile):
        ...
        if not paired:
            ...
            unzip_statement = ""
            rm_statement = "rm -rf %(infile)s %(tempdir)s" % locals()
        else:
            ...
            rm_statement = "rm -rf %(tempdir)s %(temp1)s %(temp2)s" % locals()
        ...
        statement = '''%(unzip_statement)s checkpoint; 
                       spades.py -m %%(memory)s -t %%(job_threads)s %(files_statement)s -o %(tempdir)s %(k)s %(spades_options)s; checkpoint;
                       %(move_statement)s; checkpoint;
                       %(rm_statement)s''' % locals()
        return statement
...
class Metaphlan(Idba):
    ...
    '''
    metaphlan is more of an annotation tool
    ...
    def build(self, 
        ...
        if executable == "bowtie2":
            file_statement = """cat %(infile)s                                                                                                                                                                                                           
                              | metaphlan.py -t %(method)s %(a_method)s %%(metaphlan_db)s                                                                                                                                                                             
                               --input_type multifasta --no_map --bt2_ps %%(metaphlan_bowtie2_options)s
                             """ % locals()
        elif executable == "blast":
            file_statement = """metaphlan.py -t %(method)s %(a_method)s %%(metaphlan_db)s                                                                                                                                                                            
                               --input_type multifasta  --no_map %(infile)s
                             """ % locals()

        if method == "read_map":
            statement = '''%(file_statement)s                                                                                                                                                                                                           
